<!-- Modal insertar Horario-->
<div class="modal fade" id="nuevoModalHorario" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="nuevoModalHorarioLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <h1 class="modal-title fs-5 text-light" id="nuevoModalHorarioLabel"><em> Nuevo Horario</em></h1>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form class="needs-validation" action="guardarH.php" method="post" enctype="multipart/form-data" novalidate>               
                ...
                <select name="select_ruta" id="select_ruta" class="form-control"  aria-label="Default select example" required>
                   ...
                   <?php
                    require '../config/database.php';
                     $consulta = "SELECT r.id_ruta, lugarOrigen.nombre_lugar AS lugar_origen, lugarDestino.nombre_lugar  AS lugar_destino
                     FROM ruta AS r 
                     ...
                     $select_ruta = $conectar->query($consulta);
                     //Ejecutando la consulta
                     $ruta =  mysqli_query( $conectar,$consulta);
                            while ($registroRuta = mysqli_fetch_assoc($ruta)) { ?>
                            <option value="<?php echo $registroRuta["id_ruta"]; ?>"><?=$registroRuta["lugar_origen"].'-'. $registroRuta["lugar_destino"]?></option>
                        <?php   } ?>
                </select>
                <div class="invalid-feedback">
