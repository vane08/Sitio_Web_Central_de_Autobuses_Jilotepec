...
$table = "usuario";
$id = 'id_usuario';
$campo = isset($_POST['campo_u']) ? $conectar->real_escape_string($_POST['campo_u']) : null; 
...
if ($num_rows > 0) {
    while($row =$resultado->fetch_assoc()){
       ...
       $output['data'].= '<td><div class="text-center"><a href="#" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editaModal_usuarios" data-bs-id="'.$row['id_usuario'] .'">Editar</a></div></td> ';
       $output['data'].= '<td><div class="text-center"><a href="#" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#eliminaModal_usuarios" data-bs-id="'. $row['id_usuario'] .'">Eliminar</a></td>';
       $output['data'].= '</tr>';
    }
}else{
   $output['data'].='<tr>';
